  ...
  return (
    <div className="rounded-xl border border-[#E9EAEC] bg-white p-5">
      <div className="mb-4 flex items-center gap-2">
        ...
        <p className="text-[13px] font-black uppercase tracking-wide text-[#1F2937]">Incident Types</p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-2">
        <div className="relative flex-1">
          <Tag size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF]" />
          <input
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleAdd()}
            placeholder="New incident type (e.g. Vandalism)"
            className="w-full rounded-lg border border-[#E9EAEC] py-2.5 pl-9 pr-3 text-[13px] text-[#1F2937] outline-none transition placeholder:text-[#9CA3AF] focus:border-[#3B82F6]"
          />
        </div>
        <button
          onClick={handleAdd}
          disabled={adding || !newName.trim()}
          className="flex shrink-0 items-center gap-1.5 rounded-lg bg-[#3B82F6] px-4 py-2.5 text-[12px] font-bold text-white shadow-sm transition hover:bg-[#2563EB] disabled:opacity-50"
        >
          <Plus size={14} />
          {adding ? "Adding…" : "Add"}
        </button>
      ...
      {/* List */}
      <div className="overflow-hidden rounded-lg border border-[#E9EAEC]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#F4F5F7]">
              <Tag size={16} className="text-[#D1D5DB]" />
            </div>
            <p className="text-[12px] font-semibold text-[#6B7280]">No incident types yet</p>
            <p className="text-[11px] text-[#9CA3AF]">Add your first one above.</p>
          </div>
        ) : (
          <div>
            <div className="grid grid-cols-[2fr_1fr_1fr_auto] gap-3 bg-[#F9FAFB] px-4 py-2 border-b border-[#E9EAEC]">
            ...
            {types.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className="grid grid-cols-[2fr_1fr_1fr_auto] items-center gap-3 border-b border-[#F4F5F7] px-4 py-3 last:border-0"
                >
                  {/* Name / rename input */}
                  <div className="min-w-0">
                    {isEditing ? (
                      <input
                        ...
                        className="w-full rounded-lg border border-[#3B82F6] px-2.5 py-1.5 text-[13px] text-[#1F2937] outline-none"
                      />
                    ) : (
                      <p className={`truncate text-[13px] font-semibold ${t.is_active ? "text-[#1F2937]" : "text-[#9CA3AF]"}`}>
                        {t.name}
                      </p>
                    )}
                  </div>
                  ...
                  <span>
                    {t.is_active ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-gray-500">
                        <span className="h-1.5 w-1.5 rounded-full bg-current opacity-70" />
                        Inactive
                      </span>
                    )}
