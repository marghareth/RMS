  ...
  return (
    <div>
      <div className="mb-5 flex items-start justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-[#1F2937]">Appropriations</h1>
          <p className="mt-0.5 text-[12px] text-[#9CA3AF]">Budget items allocated across PS, MOOE, and Capital Outlay</p>
        </div>
        <button
        ...
        <div className="mb-5 rounded-xl border border-[#E9EAEC] bg-white p-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div className="lg:col-span-2">
              <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[#6B7280]">Item Name *</label>
              <input
            ...
            <div>
              <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[#6B7280]">Category *</label>
              <select
                value={form.category}
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-xl border border-[#E9EAEC] bg-white px-3 py-2.5 text-[13px] outline-none focus:border-[#3B82F6]">
          <option value="">All Categories</option>
          {(["PS", "MOOE", "CO"] as const).map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-[#E9EAEC] bg-white px-3 py-2.5 text-[13px] outline-none focus:border-[#3B82F6]">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[#E9EAEC] bg-[#F9FAFB]">
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Item</th>
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Category</th>
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Fund Source</th>
                <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Appropriated</th>
                <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Disbursed</th>
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[#6B7280]">Status</th>
              </tr>
            </thead>
            <tbody>
              {items.map((a) => (
                <tr key={a.id} className="border-b border-[#F4F5F7] transition last:border-b-0 hover:bg-[#F9FAFB]">
                  <td className="px-4 py-3 text-[12px] font-bold text-[#1F2937]">{a.item_name}</td>
                  ...
                  <td className="px-4 py-3 text-[12px] text-[#374151]">{a.fund_source?.name ?? "—"}</td>
                  <td className="px-4 py-3 text-right text-[12px] font-semibold text-[#1F2937]">{fmtCurrency(a.appropriated_amount)}</td>
                  <td className="px-4 py-3 text-right text-[12px] text-[#6B7280]">{fmtCurrency(a.disbursed_amount)}</td>
                  <td className="px-4 py-3">
            ...
            {items.length > 0 && (
              <tfoot>
                <tr className="bg-[#F9FAFB]">
                  <td colSpan={3} className="px-4 py-3 text-[12px] font-black uppercase text-[#1F2937]">Total</td>
                  <td className="px-4 py-3 text-right text-[12px] font-black text-[#1F2937]">{fmtCurrency(totals.appropriated)}</td>
                  <td className="px-4 py-3 text-right text-[12px] font-black text-[#6B7280]">{fmtCurrency(totals.disbursed)}</td>
                  <td />
                </tr>
              </tfoot>
            )}
