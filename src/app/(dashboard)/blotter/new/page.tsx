...
export default function NewBlotterCasePage() {
  ...
  const suggestedHearing = addWorkingDays(new Date(incidentDate || Date.now()), 3)
    .toISOString()
    .slice(0, 10);
  ...
  async function handleSubmit() {
    ...
    // ── MOCK SUBMIT ─────────────────────────────────────────────────────
    // Simulates the create + redirect flow. Swap for the real POST call
    // below once the database is connected.
    await new Promise((r) => setTimeout(r, 500));
    setSubmitting(false);
    alert(
      `[MOCK] Case filed for ${finalComplainantName} vs ${finalRespondentName}.\nA real save will redirect to the new case's detail page.`
    );
    router.push("/blotter");
