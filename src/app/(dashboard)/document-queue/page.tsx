        ...
        <div className="lg:col-span-1">
          <div className="mb-3 flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-[#E9EAEC] bg-white py-2 pl-8 pr-3 text-[12px] text-[#1F2937] outline-none transition placeholder:text-[#9CA3AF] focus:border-[#3B82F6]"
              />
            </div>
          </div>
            ...
            {(["ACTIVE", "PENDING", "PROCESSING", "ALL"] as QueueFilter[]).map((f) => (
              <button
                ...
                className={`flex-1 rounded-lg py-1.5 text-[9px] font-bold uppercase tracking-wide transition ${
                  filter === f ? "bg-[#3B82F6] text-white" : "bg-[#F4F5F7] text-[#6B7280] hover:bg-[#E5E7EB]"
                }`}
              >
          ...
          <div className="overflow-hidden rounded-xl border border-[#E9EAEC] bg-white">
            {loading ? (
              ...
              <div className="max-h-160 divide-y divide-[#F4F5F7] overflow-y-auto">
                {items.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 px-4 py-3 text-left transition ${active ? "bg-[#3B82F6]" : "hover:bg-[#F9FAFB]"}`}
                    >
                      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${active ? "bg-blue-400" : "bg-[#F4F5F7]"}`}>
                        <FileText size={16} className={active ? "text-white" : "text-[#9CA3AF]"} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className={`truncate text-[13px] font-bold ${active ? "text-white" : "text-[#1F2937]"}`}>
                          {c.queue_number}
                        </p>
                        <p className={`truncate text-[11px] ${active ? "text-blue-100" : "text-[#9CA3AF]"}`}>
          ...
          <div className="rounded-xl border border-[#E9EAEC] bg-white p-5">
            {!selected ? (
              ...
              <div>
                <div className="mb-5 flex items-start justify-between border-b border-[#E9EAEC] pb-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-[16px] font-black uppercase tracking-wide text-[#1F2937]">{selected.queue_number}</h2>
                  ...
                  <button
                    ...
                    className="rounded-lg border border-[#E9EAEC] px-3 py-1.5 text-[12px] font-bold text-[#6B7280] transition hover:bg-[#F4F5F7]"
                  >
                    Full Details
                  </button>
                </div>
                ...
                {actionError && (
                  <div className="mb-4 rounded-lg bg-red-50 px-3 py-2.5 text-[12px] text-red-600">{actionError}</div>
                )}
                ...
                {selected.status !== "RELEASED" && selected.status !== "CANCELLED" && (
                  <div className="mb-5 flex items-center gap-2 rounded-xl bg-[#F9FAFB] border border-[#F4F5F7] px-4 py-3">
                    <Wallet size={14} className="text-[#6B7280]" />
                    <span className="text-[12px] font-semibold text-[#374151]">Payment Status</span>
                      ...
                      {(["PENDING", "PAID", "WAIVED"] as const).map((ps) => (
                        <button
                          ...
                          className={`rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase transition ${
                            selected.payment_status === ps ? "bg-[#3B82F6] text-white" : "bg-white border border-[#E9EAEC] text-[#6B7280] hover:bg-[#F4F5F7]"
                          }`}
                        >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.status === "PENDING" && (
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg bg-[#3B82F6] px-4 py-2.5 text-[12px] font-bold text-white shadow-sm transition hover:bg-[#2563EB] disabled:opacity-50"
                      >
                        <PlayCircle size={14} />
                        Start Processing
                      </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-[#E9EAEC] px-4 py-2.5 text-[12px] font-bold text-red-500 transition hover:bg-red-50 disabled:opacity-50"
                    >
                      <XCircle size={14} />
                      Cancel Request
                    </button>
