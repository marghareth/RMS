...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-[#E9EAEC] rounded-xl px-3 py-2 shadow-lg text-[11px]">
      ...
      <p className="text-[#0B6E4F]">{payload[0]?.value} persons</p>
    </div>
  );
}
...
export default function RegistriesReportPage() {
  ...
  const TABS: { key: Tab; label: string; icon: any; count: number; color: string }[] = [
    { key: "seniors", label: "Senior Citizens", icon: Users,          count: data.seniors.total, color: "text-[#B45309]" },
    { key: "pwd",     label: "PWD",             icon: Accessibility,  count: data.pwd.total,     color: "text-[#3E5C76]" },
    { key: "fourps",  label: "4Ps Beneficiaries",icon: Heart,         count: data.fourPs.total,  color: "text-[#0B6E4F]" },
  ];
    ...
    <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[#F4F5F7] flex items-center justify-center">
                <BookOpen size={18} className="text-[#0E7490]" />
              </div>
              <div>
                <h1 className="text-[17px] font-bold text-[#1B2430] uppercase tracking-wide">Special Registries</h1>
          ...
          return (
            <button
              ...
              className={`rounded-xl border px-5 py-4 text-left transition
                ${tab === t.key ? "border-[#0B6E4F]/40 bg-[#E8F3EE]/60" : "border-[#E9EAEC] bg-white hover:border-[#0B6E4F]/30 hover:bg-[#E8F3EE]/50"}`}
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white">
                  <Icon size={14} className={t.color} />
                </div>
                <span className={`text-[10px] font-bold uppercase tracking-wide ${tab === t.key ? "text-[#0B6E4F]" : "text-[#9CA3AF]"}`}>{t.label}</span>
              </div>
              <p className="text-[32px] font-bold leading-none tabular-nums text-[#1B2430]">
                {t.count}
              </p>
            </button>
          );
        ...
        <div className="space-y-5">
              ...
              <div className="space-y-3 pt-2">
                {data.seniors.byPurok.map((p, i) => (
                  <div key={p.purok} className="flex items-center gap-3">
                    ...
                    <span className="text-[12px] font-bold tabular-nums text-[#1B2430] min-w-7 text-right">{p.count}</span>
          ...
          <div className="bg-white rounded-xl border border-[#E9EAEC] overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E9EAEC] bg-[#F9FAFB]">
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#1B2430]">Senior Citizen Registry (Sample)</p>
            </div>
            ...
            {data.seniorList.map((s, i) => (
              <div key={s.id} className={`grid grid-cols-[2fr_1fr_1fr_1fr] gap-4 px-5 py-3 border-b border-[#F4F5F7] items-center ${i % 2 !== 0 ? "bg-[#FAFAFA]" : ""}`}>
                <span className="text-[12px] font-semibold text-[#1B2430]">{s.name}</span>
                <span className="text-[12px] tabular-nums text-[#374151]">{s.age}</span>
      ...
      {tab === "pwd" && (
        <div className="space-y-5">
                ...
                <div className="flex-1 space-y-1.5">
                  {data.pwd.byType.map((t, i) => (
                    <div key={t.type} className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-bold tabular-nums text-[#1B2430]">{t.count}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl border border-[#E9EAEC] overflow-hidden">
            <div className="px-5 py-4 border-b border-[#E9EAEC] bg-[#F9FAFB]">
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#1B2430]">PWD Registry (Sample)</p>
            </div>
